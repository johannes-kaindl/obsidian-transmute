// uebernommen aus 3d-codeblocks/scripts/lib/vault.ts, 2026-08-16
/**
...
export function buildVault(spec: VaultSpec): string[] {
  ...
  // ABWEICHUNG zur Vorlage (2026-08-16): dort ist der Generator-Aufruf ein PFLICHTschritt
  // und heisst `make-models.mjs` — ein 3d-codeblocks-Spezifikum in einer Datei, die sich
  // „die pluginneutrale Haelfte" nennt. Hier optional und generisch benannt: ein Fixture
  // aus reinen Markdown-Notizen braucht keinen Generator.
  const generator = join(fixtureDir, "make-fixture.mjs");
  if (existsSync(generator)) {
    const erzeugt = execFileSync("node", [generator, vaultDir], { encoding: "utf-8" });
    log.push("Fixture-Generator:" + erzeugt.replace(/\n$/, "").replace(/\n/g, "\n   "));
  }
