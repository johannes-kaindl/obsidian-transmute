...
export async function setWindowSize(cdp: Cdp, width: number, height: number): Promise<boolean> {
  // ABWEICHUNG zur Vorlage (2026-08-16): im Vollbild schluckt macOS `setSize` — die
  // Vorlage setzte und nahm an. Hier erst raus aus dem Vollbild, dann setzen, dann die
  // TATSAECHLICHE Groesse zurueckmelden: sie ist durch den Bildschirm begrenzt (auf dem
  // Aufnahmegeraet 949 px nutzbare Hoehe), und ein Rezept, das mit 1000 rechnet, schneidet
  // sonst ab.
  const gemessen = await cdp.evaluate<string>(`
    ...
    return win.getSize().join("x");
  `);
  const [w, h] = gemessen.split("x").map(Number);
  if (w !== width || h !== height) {
    console.log(`  Fenster: ${gemessen} statt ${width}x${height} (durch den Bildschirm begrenzt)`);
  }
  return true;
}
