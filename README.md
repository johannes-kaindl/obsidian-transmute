...
- **You decide about risky patterns you wrote yourself.** A hand-written pattern that trips the backtracking heuristic is now a warning with a reason, not a refusal: "Run it anyway" releases it for exactly that pattern. Change one character and the warning is back — a release for `(a+)+b` says nothing about `(a+)+bc`.
